...
std::atomic<PAYLOAD_TYPE> sum;

std::atomic<long long> cumulative_inserts;
std::atomic<long long> cumulative_lookups;

std::atomic<double> cumulative_insert_time;
std::atomic<double> cumulative_lookup_time;

// Concurrent processing
void proc(int init_num_keys, std::string lookup_distribution, int num_lookups_per_batch, int num_inserts_per_batch){
  int i = init_num_keys;
  while (true) {
    // Do lookups
    ...
    // Do inserts
    int num_actual_inserts = num_inserts_per_batch;
...
int main(int argc, char* argv[]) {
  ...
  // Read keys from file
  keys = new KEY_TYPE[total_num_keys];
  ...
  values = new std::pair<KEY_TYPE, PAYLOAD_TYPE>[init_num_keys];
  ...
  // Run workload
  int i = init_num_keys;
  std::atomic_init<long long>(&cumulative_inserts, 0);
  std::atomic_init<long long>(&cumulative_lookups, 0);
  ...
  std::atomic_init<double>(&cumulative_insert_time, 0);
  std::atomic_init<double>(&cumulative_lookup_time, 0);

  auto workload_start_time = std::chrono::high_resolution_clock::now();
  int batch_no = 0;
  std::atomic_init<PAYLOAD_TYPE>(&sum, 0);
  ...
  for(int i = 0; i < num_threads; i++){
    threads[i] = std::thread(proc, init,lookup_distribution, num_lookups_per_batch / num_threads, std::min(num_inserts_per_batch / num_threads, total_num_keys / num_threads - i));
    init += std::min(num_inserts_per_batch / num_threads, total_num_keys / num_threads - init);
  }
